...
- 结束页蓝白根背景从切入的第一帧起必须完全不透明，不能淡入，不能透出上一段 UI；
- 卡片可在不透明背景上用 0.3–0.5 秒轻微上移、缩放和淡入；
- Logo、分隔线、署名依次出现；
- 至少留下约 1 秒完整静止画面；
...
- 使用 `assets/templates/brand-end-card.html` 作为默认自包含子合成；
- 禁止从其他视频中截取结束段，因为透明转场可能带入该视频的手机、截图或业务页面；
- 禁止给结束页根节点、全屏背景或根 composition 添加从 `opacity: 0` 开始的动画；
- 如需柔和转场，可让前景卡片淡入，或使用完全不透明的全屏遮罩切换；
- 在结束页第 1 帧、`+0.1s`、`+0.3s` 抽帧检查，三帧都不得出现上一场景残影。
