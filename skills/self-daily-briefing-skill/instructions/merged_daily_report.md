...
生成默认完整 Morning Brief：包含综合早报、财经早报、科技早报、AI 深度日报、美股股票早报。默认直接在对话里输出完整日报；只有用户明确要求 PDF、Markdown、HTML 或保存文件时，才导出文件。

## 严格规则

1. 先生成或读取各板块源文件。
2. `general_report.md`、`finance_report.md`、`tech_report.md`、`ai_daily_report.md` 是标准日报稿的事实来源，必须原文贴入。
3. 不要对前四个板块做二次摘要、改写或统一文风。
4. 如果用户给了某个板块的完整内容，按用户版本原文覆盖。
5. 如果缺少某个源文件或当天数据不足，明确写“数据缺口”，不要编造。
6. 最终默认直接在对话里输出完整日报正文。
7. 如果用户要求 PDF，PDF 要保留 Markdown 的原始阅读结构和链接，优先用 HTML 阅读页导出，不要重新设计版式。
